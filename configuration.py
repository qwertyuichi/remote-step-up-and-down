...
def get_default_file_and_hash():
    return 'configuration.yml',\
        '081fe5acc8c4d3644556134a7e9c70f9'
...
@dataclasses.dataclass(frozen=True)
class Debug:
    streaming_enable: bool = True
    streaming_port: int = 12345
    imshow_enable: bool = True
    save_movie_enable: bool = True
    movie_folder_path: str = '/home/yuichi/Share/log'


@dataclasses.dataclass(frozen=True)
class SerialCommunication:
    primary_port: str = '/dev/ttyTHS0'
    secondary_port: str = '/dev/ttyTHS1'
...
@dataclasses.dataclass(frozen=True)
class _Config:
    realsense: Realsense = dataclasses.field(init=False)
    roi: Roi = dataclasses.field(init=False)
    depth_max: int = 1000
    distance_min: int = 390
    distance_max: int = 410
    angle_min: float = -2.0
    angle_max: float = 2.0
    debug: Debug = dataclasses.field(init=False)
    serial_communication: SerialCommunication = dataclasses.field(init=False)

    def __post_init__(self):
        super().__setattr__('realsense', Realsense())
        super().__setattr__('roi', Roi())
        super().__setattr__('debug', Debug())
        super().__setattr__('serial_communication', SerialCommunication())
